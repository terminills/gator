...
async def test_list_segments_by_status(db_session, test_persona):
    ...
    await service.update_segment(str(inactive.id), status=SegmentStatus.INACTIVE)

    active_segments = await service.list_segments(
        persona_id=str(test_persona.id), status=SegmentStatus.ACTIVE
    )
...
@pytest.mark.asyncio
async def test_get_personalized_content_by_segment(
    db_session, test_persona, test_content
):
