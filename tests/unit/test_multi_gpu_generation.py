...
class TestMultiGPUImageGeneration:
    ...
    async def test_generate_images_batch_single_prompt(self, ai_model_manager):
        ...
        with patch.object(
            ai_model_manager, 'generate_image', 
    ...
    async def test_generate_images_batch_multiple_prompts_no_gpu(
        ...
        with patch("torch.cuda.is_available", return_value=False):
            with patch.object(
                ai_model_manager, '_generate_image_local',
                new_callable=AsyncMock
    ...
    async def test_generate_images_batch_with_multi_gpu(self, ai_model_manager):
        ...
        with patch("torch.cuda.is_available", return_value=True):
            with patch("torch.cuda.device_count", return_value=2):
                with patch.object(
                    ai_model_manager, '_generate_image_on_device',
                    new_callable=AsyncMock
    ...
    @pytest.mark.asyncio
    async def test_generate_image_on_device_adds_device_id(
        self, ai_model_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_images_batch_handles_exceptions(
        self, ai_model_manager
    ):
