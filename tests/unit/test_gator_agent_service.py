...
class TestGatorAgentService:
    ...
    async def test_gator_dns_help(self, agent):
        ...
        assert ("dns management tab" in response.lower() or 
                "auto-setup" in response.lower())
    ...
    async def test_gator_error_handling(self, agent):
        ...
        assert any(phrase in response for phrase in [
            "Hold up", "not right", "broken", "gonna fix"
        ])
    ...
    async def test_long_message_handling(self, agent):
        ...
        long_message = "Hey Gator, " * 20 + "can you help me understand how to set up personas?"
...
class TestGatorKnowledgeBase:
    ...
    async def test_troubleshooting_responses(self, agent):
        ...
        assert any(word in slow_response.lower() for word in ["resources", "hardware", "cpu", "ram"])
    ...
    async def test_system_status_help(self, agent):
        ...
        assert any(word in response.lower() for word in ["status", "dashboard", "system"])
    ...
    async def test_unknown_topic_fallback(self, agent):
        ...
        assert any(phrase in response.lower() for phrase in [
            "not sure", "more specific", "what do you need"
        ])
