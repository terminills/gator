...
class TestAIModelsVideoGeneration:
    ...
    def test_frame_by_frame_model_features(self, mock_video_models):
        ...
        fbf_model = next(
            m for m in mock_video_models.available_models["video"]
    ...
    async def test_generate_video_with_frame_by_frame(self, mock_video_models):
        ...
        with patch('backend.services.ai_models.AIModelManager._generate_video_frame_by_frame') as mock_generate:
    ...
    async def test_synchronize_audio_to_video(self, mock_video_models):
        ...
        with patch('backend.services.video_processing_service.VideoProcessingService.synchronize_audio_with_video') as mock_sync:
    ...
    async def test_create_video_storyboard(self, mock_video_models):
        ...
        with patch('backend.services.video_processing_service.VideoProcessingService.create_storyboard') as mock_storyboard:
    ...
    async def test_frame_by_frame_with_quality_settings(self, mock_video_models):
        ...
        with patch('backend.services.video_processing_service.VideoProcessingService.generate_frame_by_frame_video') as mock_gen:
...
class TestVideoModelInitialization:
    ...
    async def test_frame_by_frame_always_available(self):
        ...
        fbf_model = next(
            m for m in manager.available_models["video"]
            if m["name"] == "frame-by-frame-generator"
        )
