...
async def test_schema_status_identifies_tables(db_admin_service):
    ...
    expected_model_tables = ["personas", "users", "conversations", "messages", 
                            "ppv_offers", "content", "rss_feeds", "feed_items"]
    
    for table in expected_model_tables:
        assert table in result["model_tables"], f"Expected model table '{table}' not found"
