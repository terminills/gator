...
def test_pyproject_toml_pytorch_version():
    ...
    assert (
        "torch==2.3.1+rocm5.7" in content
    ), "pyproject.toml must specify torch==2.3.1+rocm5.7 in rocm extras"
    assert (
        "torchvision==0.18.1+rocm5.7" in content
...
def test_no_conflicting_versions():
    ...
    for filename in files_to_check:
        ...
        for pattern in outdated_patterns:
            ...
            if filename == "setup_ai_models.py" and pattern.startswith(
                r"torch>=2\.0\.0"
            ):
                continue

            assert len(matches) == 0, f"{filename} contains outdated pattern: {pattern}"
