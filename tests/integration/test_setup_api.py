...
class TestSetupAPI:
    ...
    def test_update_configuration_validation(self, test_client):
        ...
        config = {"smtp_port": 99999}  # Invalid port
    ...
    def test_update_configuration_success(self, test_client):
        ...
        config = {"environment": "test", "debug": True, "log_level": "INFO"}
    ...
    def test_ai_models_status_version_info(self, test_client):
        ...
        assert (
            "installed_versions" in data
        ), "Response should include installed_versions"
        assert "required_versions" in data, "Response should include required_versions"
        assert (
            "compatibility_note" in data
        ), "Response should include compatibility_note"
        ...
        assert (
            "2.3.1" in required["torch"]
        ), "Required torch version should be 2.3.1 for MI-25 GPU"
        assert (
            "rocm5.7" in required["torch"]
        ), "Required torch version should include ROCm 5.7"

        # Verify other ML dependencies
        assert "diffusers" in required
        assert (
            ">=0.28.0" in required["diffusers"]
        ), "diffusers should require >=0.28.0 for PyTorch 2.3.1"

        assert "transformers" in required
        assert (
            ">=4.41.0" in required["transformers"]
        ), "transformers should require >=4.41.0 for PyTorch 2.3.1"

        assert "accelerate" in required
        assert (
            ">=0.29.0" in required["accelerate"]
        ), "accelerate should require >=0.29.0 for PyTorch 2.3.1"

        assert "huggingface_hub" in required
        assert (
            ">=0.23.0" in required["huggingface_hub"]
        ), "huggingface_hub should require >=0.23.0"

        # Verify compatibility note mentions MI-25
        assert (
            "MI-25" in data["compatibility_note"]
        ), "Compatibility note should mention MI-25 GPU"
        assert (
            "2.3.1" in data["compatibility_note"]
        ), "Compatibility note should mention PyTorch 2.3.1"
    ...
    def test_ai_models_analyze(self, test_client):
        ...
        if data["success"]:
            assert (
                "System Information" in data["output"]
                or "Analyzing system" in data["output"]
    ...
    def test_ai_models_install_response_structure(self, test_client):
        ...
        request = {"model_names": ["non-existent-test-model"], "model_type": "text"}
        ...
        if response.status_code == 200:
            ...
            assert (
                "stdout" in data
            ), "Response should include stdout for installation logs"
