...
class TestDirectMessagingAPIIntegration:
    ...
    async def test_create_conversation_workflow(self, test_client, sample_user, sample_persona):
...
class TestQueueManagement:
    ...
    def test_queue_logic_concepts(self):
        ...
        for conv in conversations:
            ...
            if (conv["last_persona_message_at"] is None or 
                conv["last_persona_message_at"] < cutoff):
        ...
        needs_response.sort(key=lambda x: (
            -x["priority"],  # Higher priority first
            x["last_persona_message_at"] or datetime.min.replace(tzinfo=timezone.utc)  # FIFO, nulls first
        ))
