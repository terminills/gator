...
def test_bash_syntax():
    ...
    result = subprocess.run(
        ["bash", "-n", "server-setup.sh"], capture_output=True, text=True
    )
...
def test_mi25_detection_logic():
    ...
    assert (
        "vega.*10" in content.lower() or "vega 10" in content.lower()
    ), "Missing Vega 10 detection"
...
def test_amdgpu_install_utility():
    ...
    assert (
        "amdgpu-install_5.7.50701-1_all.deb" in content
    ), "Missing amdgpu-install package"
    assert (
        "--usecase=rocm,hiplibsdk,dkms" in content
    ), "Missing ROCm use-case configuration"
