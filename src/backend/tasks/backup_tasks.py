...
def create_automated_backup() -> Dict[str, Any]:
    ...
    try:
        ...
        metadata = {
            ...
            "content_size_mb": (
                _get_file_size_mb(content_backup_file) if content_result else 0
            ),
        ...
        for backup_path in backup_dir.iterdir():
            ...
            if metadata_file.exists():
                ...
                backup_date = datetime.fromisoformat(
                    metadata.get("created_at", "2000-01-01")
                )
...
def _backup_database(output_file: Path) -> bool:
    ...
    try:
        ...
        if "sqlite" in database_url:
            ...
            # Compress the dump
            subprocess.run(["gzip", str(output_file.with_suffix(".sql"))], check=True)
            ...
            with open(output_file, "wb") as f:
                subprocess.run(
                    ...
                    input=subprocess.run(
                        ["pg_dump", database_url], stdout=subprocess.PIPE
                    ).stdout,
                    stdout=f,
                    check=True,
                )
...
def _backup_content(output_file: Path) -> bool:
    ...
    try:
        content_dir = Path(
            getattr(settings, "CONTENT_STORAGE_PATH", "generated_content")
        )
...
def restore_backup(backup_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "failed", "error": str(e), "backup_path": backup_path}
    ...
    try:
        ...
        if "sqlite" in database_url:
            ...
            with open(backup_file, "rb") as f:
                subprocess.run(["gunzip", "-c"], stdin=f, stdout=subprocess.PIPE)
        ...
        # For PostgreSQL
            ...
            subprocess.run(["gunzip", "-c", str(backup_file)], stdout=subprocess.PIPE)
...
def _restore_content(backup_file: Path) -> bool:
    ...
    try:
        ...
        subprocess.run(
            ["tar", "xzf", str(backup_file), "-C", str(content_dir.parent)], check=True
        )
