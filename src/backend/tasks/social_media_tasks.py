...
@app.task(base=AsyncTask, bind=True, name='backend.tasks.social_media_tasks.publish_scheduled_post')
async def publish_scheduled_post(self, schedule_id: str, post_data: Dict[str, Any]) -> Dict[str, Any]:
    """
        ...
        async with database_manager.get_session() as session:
            ...
            stmt = select(ContentModel).where(
                and_(
                    ...
                    ContentModel.metadata['schedule_time'].astext.cast(datetime) <= current_time
                )
            )
            ...
            for content in scheduled_content:
                try:
                    ...
                    logger.error(f"Error processing scheduled content {content.id}: {str(e)}")
                    error_count += 1
...
def batch_publish_content(content_ids: List[str], platforms: List[str]) -> Dict[str, Any]:
    """
...
async def _batch_publish_async(content_ids: List[str], platforms: List[str]) -> Dict[str, Any]:
            ...
            for content_id in content_ids:
                try:
                    ...
                    results.append({
                        'content_id': content_id,
                        'status': 'success',
                        'results': [{'platform': r.platform, 'status': r.status} for r in post_results]
                    })
