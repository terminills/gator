...
class OrchestrationRequest(BaseModel):
    ...
    additional_context: Optional[Dict[str, Any]] = Field(
        None, description="Extra context"
    )
...
class LearningRequest(BaseModel):
    ...
    outcome_metadata: Optional[Dict[str, Any]] = Field(
        None, description="Outcome details"
    )
...
async def orchestrate_decision(
    ...
    try:
        ...
        if not context:
            raise HTTPException(
                status_code=404, detail=f"ACD context {request.context_id} not found"
            )
...
async def orchestrate_and_execute(
    ...
    try:
        ...
        return ExecutionResponse(
            ...
            decision_type=decision.decision_type.value,
            message=(
                f"Decision executed: {decision.decision_type.value}"
                if executed
    ...
    try:
        ...
        total_decisions = len(
            [
                c
                for c in contexts
                if c.ai_metadata and "orchestration_decision" in c.ai_metadata
            ]
        )
