...
router = APIRouter(prefix="/api/v1/civitai", tags=["civitai"])
...
class CivitAISearchRequest(BaseModel):
    ...
    model_types: Optional[List[str]] = Field(
        None, description="Filter by model types (Checkpoint, LORA, etc.)"
    )
    base_models: Optional[List[str]] = Field(
        None, description="Filter by base models (SDXL 1.0, SD 1.5, etc.)"
    )
...
class CivitAIDownloadRequest(BaseModel):
    ...
    file_type: Optional[str] = Field(
        None, description="File type filter (Model, VAE, Pruned Model)"
    )
    output_directory: Optional[str] = Field(
        None, description="Custom output directory (relative to models path)"
    )
...
async def search_models(
    ...
    model_types: Optional[str] = Query(
        None, description="Comma-separated model types (Checkpoint,LORA,etc.)"
    ),
    base_models: Optional[str] = Query(
        None, description="Comma-separated base models (SDXL 1.0,SD 1.5,etc.)"
    ),
    limit: int = Query(20, ge=1, le=100, description="Number of results"),
    page: int = Query(
        1, ge=1, description="Page number (ignored when query is provided)"
    ),
    cursor: Optional[str] = Query(
        None, description="Cursor for pagination (use with query parameter)"
    ),
    sort: str = Query("Highest Rated", description="Sort order"),
    period: str = Query("AllTime", description="Time period"),
    nsfw: bool = Query(
        True, description="Include NSFW models (enabled by default for private server)"
    ),
):
    """
...
async def list_models(
    ...
    try:
        ...
        if not allow_nsfw and nsfw:
            raise HTTPException(
                status_code=403, detail="NSFW models are disabled in settings"
            )

        # Parse model types and base models
        type_list = (
            model_types.split(",") if model_types and model_types.strip() else None
        )
        base_list = (
            base_models.split(",") if base_models and base_models.strip() else None
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch models from CivitAI: {str(e)}"
        )
...
async def get_model_details(model_id: int):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch model details: {str(e)}"
        )
...
async def get_model_version(version_id: int):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch version details: {str(e)}"
        )
...
async def list_local_civitai_models():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to list local models: {str(e)}"
        )
