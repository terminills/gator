    ...
    try:
        ...
        for trace in traces:
            recent_errors.append({
                ...
                "acd_context_id": str(trace.acd_context_id) if trace.acd_context_id else None,
            })
        
        # Calculate success rate
        success_rate = (successful / total_generations * 100) if total_generations > 0 else 0.0
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get AI activity: {str(e)}")
...
async def get_generation_attempts(
    ...
    status: Optional[str] = Query(None, description="Filter by status (success, failed, fallback)"),
    ...
    try:
        ...
        for content, persona in rows:
            ...
            if acd_context_id:
                try:
                    ...
                    acd_id = UUIDType(acd_context_id) if isinstance(acd_context_id, str) else acd_context_id
                    acd_context = await acd_service.get_context(acd_id)
                except Exception as e:
                    logger.warning(f"Failed to fetch ACD context {acd_context_id}: {str(e)}")
            
            attempts.append(ContentGenerationAttempt(
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get generation attempts: {str(e)}")
...
async def get_ai_model_activity(
    ...
    try:
        ...
        for content in contents:
            ...
            gen_time = gen_params.get("generation_time_seconds") or gen_params.get("total_time_seconds")
            if gen_time:
        ...
        for stats in model_stats.values():
            ...
            activities.append(AIModelActivity(
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get AI model activity: {str(e)}")
...
async def get_acd_contexts(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get ACD contexts: {str(e)}")
