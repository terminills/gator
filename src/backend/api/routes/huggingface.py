...
class HuggingFaceSearchRequest(BaseModel):
    ...
    task: Optional[str] = Field(
        None, description="Filter by task (e.g., text-to-image)"
    )
    library: Optional[str] = Field(
        None, description="Filter by library (e.g., diffusers)"
    )
...
class HuggingFaceDownloadRequest(BaseModel):
    ...
    repo_id: str = Field(
        ...,
        description="HuggingFace repository ID (e.g., 'stabilityai/stable-diffusion-xl-base-1.0')",
    )
    filename: Optional[str] = Field(
        None, description="Specific file to download (if None, downloads entire repo)"
    )
...
async def search_models(
    ...
    task: Optional[str] = Query(
        None, description="Filter by task (e.g., text-to-image, text-generation)"
    ),
    library: Optional[str] = Query(
        None, description="Filter by library (e.g., diffusers, transformers)"
    ),
    tags: Optional[str] = Query(None, description="Comma-separated tags to filter by"),
    limit: int = Query(20, ge=1, le=100, description="Number of results"),
    sort: str = Query(
        "downloads", description="Sort by (downloads, likes, created_at, lastModified)"
    ),
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to search models: {str(e)}"
        )
...
async def get_popular_diffusion_models(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch popular models: {str(e)}"
        )
...
async def get_popular_lora_models(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch LoRA models: {str(e)}"
        )
...
async def get_popular_text_generation_models(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch text generation models: {str(e)}"
        )
