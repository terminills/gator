...
class CreateDNSRecordRequest(BaseModel):
    ...
    ttl: int = Field(
        default=3600, ge=300, le=86400, description="Time to live in seconds"
    )
...
class SetupPlatformRequest(BaseModel):
    ...
    provider: DNSProvider = Field(
        default=DNSProvider.GODADDY, description="DNS provider"
    )
...
class DNSProviderConfig(BaseModel):
    ...
    environment: str = Field(
        default="production", description="Environment (production/ote)"
    )
...
async def delete_dns_record(
    ...
    try:
        success = await dns_service.delete_dns_record(
            domain, record_type, record_name, provider
        )
...
async def validate_dns_propagation(
    ...
    try:
        ...
        success = await dns_service.validate_dns_propagation(
            domain=domain, expected_ip=expected_ip, timeout=timeout
        )
...
async def list_dns_providers():
    ...
    godaddy_configured = bool(
        getattr(settings, "GODADDY_API_KEY", None)
        and getattr(settings, "GODADDY_API_SECRET", None)
    )
    ...
    return {"providers": providers, "default_provider": "godaddy"}
