...
@router.post("/", response_model=InteractiveContentSchema, status_code=status.HTTP_201_CREATED)
async def create_interactive_content(
    request: InteractiveContentCreate,
    ...
    try:
        ...
        logger.info(f"Created interactive content: id={content.id}, type={request.content_type}")
...
async def list_interactive_content(
    ...
    content_type: Optional[InteractiveContentType] = Query(None, description="Filter by content type"),
    status: Optional[InteractiveContentStatus] = Query(None, description="Filter by status"),
...
@router.post("/{content_id}/respond", response_model=InteractiveContentResponseSchema, status_code=status.HTTP_201_CREATED)
async def submit_response(
    ...
    try:
        ...
        logger.info(f"Submitted response to interactive content: content_id={content_id}")
