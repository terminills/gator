...
async def list_installed_models(
    model_type: Optional[str] = Query(
        None, description="Filter by model type (Checkpoint, LORA, etc.)"
    ),
    source: Optional[str] = Query(
        None, description="Filter by source (civitai, huggingface, local)"
    ),
    base_model: Optional[str] = Query(
        None, description="Filter by base model (SD 1.5, SDXL 1.0, etc.)"
    ),
...
async def enrich_model_from_civitai(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch CivitAI data: {str(e)}"
        )


@router.post(
    "/{model_id}/enrich-from-huggingface", response_model=InstalledModelResponse
)
async def enrich_model_from_huggingface(
    model_id: str,
    huggingface_repo_id: str = Query(
        ...,
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch HuggingFace data: {str(e)}"
        )
...
async def add_model_trigger_word(
    ...
    if trigger_word.lower().strip() not in [
        t.lower().strip() for t in current_triggers
    ]:
...
async def scan_models_directory(
    directory: Optional[str] = Query(
        None, description="Directory to scan (default: ai_model_path setting)"
    ),
    ...
    return {"success": True, "directory": directory, **results}
