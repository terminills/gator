...
async def get_creator_dashboard(
    days: int = Query(default=7, ge=1, le=90, description="Days to include in statistics"),
    db: AsyncSession = Depends(get_db_session),
):
    """
    ...
    try:
        ...
        # Recent activity
        recent_activity = []
        for content in sorted(recent_content, key=lambda c: c.created_at, reverse=True)[:10]:
        ...
        analytics = {
            ...
            "period_metrics": {
                ...
                "avg_quality_score": round(
                    ...
                    len([c for c in content_items if c.quality_score]), 2
                ) if content_items else 0,
                "content_types": {
                    content_type: len([c for c in content_items if c.content_type == content_type])
...
async def get_content_suggestions(
    persona_id: Optional[UUID] = Query(None, description="Persona to get suggestions for"),
    ...
    try:
        ...
        for topic_data in trending:
            ...
            if persona_id:
                ...
                if persona and topic_data["topic"].lower() in [t.lower() for t in persona.content_themes]:
...
async def optimize_persona(
    ...
    try:
        ...
        # Quality analysis
        if recent_content:
            avg_quality = sum(c.quality_score for c in recent_content if c.quality_score) / len(recent_content)
            if avg_quality < 70:
        ...
        if not persona.style_preferences or len(persona.style_preferences) < 3:
            ...
            # Apply optimization
            persona.style_preferences = {**persona.style_preferences, **suggested_styles}
            await db.commit()
            
            changes_applied.append({
                "type": "style_enhancement",
                "changes": suggested_styles
            })
