...
class TextAnalysisRequest(BaseModel):
    ...
    text: str = Field(..., description="Text content to analyze", min_length=1, max_length=10000)
...
class CommentAnalysisRequest(BaseModel):
    ...
    comments: List[Dict[str, Any]] = Field(..., description="List of comments to analyze")
...
class EngagementAnalysisRequest(BaseModel):
    ...
    engagement_data: Dict[str, Any] = Field(..., description="Engagement metrics (likes, comments, etc.)")
...
async def analyze_text_sentiment(
    ...
    try:
        ...
        logger.info(f"Text sentiment analyzed: sentiment={result['sentiment_label']} confidence={result['confidence']}")
...
async def analyze_social_comments(
    ...
    try:
        ...
        logger.info(f"Comments analyzed: count={result['total_comments']} sentiment={result.get('overall_sentiment')}")
...
async def analyze_post_engagement(
    ...
    try:
        ...
        logger.info(f"Engagement analyzed: overall_sentiment={result['overall_sentiment']}")
...
async def get_sentiment_trends(
    ...
    try:
        service = SentimentAnalysisService(session)
        result = await service.track_sentiment_trends(
            persona_id,
            timedelta(days=days)
        )
        
        logger.info(f"Sentiment trends retrieved: persona_id={persona_id} trend={result['sentiment_trend']['direction']}")
...
async def compare_competitor_sentiment(
    ...
    try:
        ...
        logger.info(f"Competitor sentiment compared: persona_id={request.persona_id} competitors={len(request.competitor_ids)}")
