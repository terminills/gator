...
@router.post("/", response_model=AudienceSegmentSchema, status_code=status.HTTP_201_CREATED)
async def create_segment(
    request: AudienceSegmentCreate,
    ...
    try:
        ...
        logger.info(f"Created audience segment: id={segment.id}, name={request.segment_name}")
...
async def list_segments(
    ...
    strategy: Optional[PersonalizationStrategy] = Query(None, description="Filter by strategy"),
...
@router.delete("/{segment_id}/members/{user_id}", status_code=status.HTTP_204_NO_CONTENT)
...
@router.post("/personalized", response_model=PersonalizedContentSchema, status_code=status.HTTP_201_CREATED)
async def create_personalized_content(
    request: PersonalizedContentCreate,
    session: AsyncSession = Depends(get_db_session)
):
    """
