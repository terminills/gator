...
from backend.models.conversation import ConversationModel, ConversationCreate, ConversationResponse, ConversationStatus
from backend.models.message import MessageModel, MessageCreate, MessageResponse, MessageSender, MessageType
from backend.models.ppv_offer import PPVOfferModel, PPVOfferCreate, PPVOfferResponse, PPVOfferStatus, PPVOfferType
...
class DirectMessagingService:
    ...
    async def get_user_conversations(
        ...
        try:
            ...
            stmt = stmt.order_by(desc(ConversationModel.last_message_at)).offset(skip).limit(limit)
    ...
    async def get_next_conversation_for_persona_response(
        ...
        """
        try:
            cutoff_time = datetime.now(timezone.utc) - timedelta(hours=max_hours_since_last_response)
    ...
    async def accept_ppv_offer(self, offer_id: uuid.UUID) -> PPVOfferResponse:
        ...
        try:
            ...
            await self.db.execute(
                ...
                .where(PPVOfferModel.id == offer_id)
                .values(
                    status=PPVOfferStatus.ACCEPTED,
                    accepted_at=now,
                    updated_at=now
                )
            )
    ...
    async def _verify_conversation_participants(self, user_id: uuid.UUID, persona_id: uuid.UUID) -> None:
