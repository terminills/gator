...
class SetupService:
    ...
    def get_setup_status(self) -> Dict[str, Any]:
        ...
        status["configured_sections"]["database"] = bool(
            ...
            not config.get("DATABASE_URL", "").startswith("sqlite")
        )
        
        # AI Models
        status["configured_sections"]["ai_models"] = bool(
            config.get("OPENAI_API_KEY") == "***CONFIGURED***" or
            config.get("ANTHROPIC_API_KEY") == "***CONFIGURED***"
        )
        
        # Social Media
        status["configured_sections"]["social_media"] = bool(
            config.get("FACEBOOK_API_KEY") == "***CONFIGURED***" or
            config.get("INSTAGRAM_API_KEY") == "***CONFIGURED***"
        )
        ...
        status["configured_sections"]["security"] = bool(
            ...
            config.get("SECRET_KEY") != "your_super_secret_key_change_in_production"
        )
