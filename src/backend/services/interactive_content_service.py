...
class InteractiveContentService:
    ...
    async def create_content(
        ...
        logger.info(f"Created interactive content: id={content.id}, type={content_type}")
        return content
    ...
    async def publish_content(self, content_id: str) -> Optional[InteractiveContentModel]:
        """
    ...
    async def submit_response(
        ...
        if content.content_type == InteractiveContentType.POLL.value and "option_id" in response_data:
            ...
            if content.options:
                ...
                for option in updated_options:
                    if total_votes > 0:
                        option["percentage"] = round((option["votes"] / total_votes) * 100, 2)
    ...
    async def get_content_stats(self, content_id: str) -> Dict[str, Any]:
        ...
        if content.content_type == InteractiveContentType.POLL.value and content.options:
    ...
    async def expire_old_content(self, content_type: Optional[InteractiveContentType] = None) -> int:
        """
        ...
        if content_type:
            query = query.where(InteractiveContentModel.content_type == content_type.value)
