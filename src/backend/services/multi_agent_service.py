...
class MultiAgentService:
    ...
    async def route_task_to_agent(
        self, routing_request: AgentRoutingRequest
    ) -> AgentRoutingResponse:
        """
        ...
        try:
            ...
            stmt = select(AgentModel).where(
                and_(
                    AgentModel.status.in_(
                        [AgentStatus.IDLE.value, AgentStatus.ACTIVE.value]
                    ),
            ...
            for agent in candidates:
                ...
                score += load_score
                reasons.append(
                    f"{agent.current_load}/{agent.max_concurrent_tasks} tasks"
                )
                ...
                scored_candidates.append(
                    {"agent": agent, "score": score, "reasons": reasons}
                )
    ...
    async def create_task(
        self, task_data: AgentTaskCreate, auto_assign: bool = True
    ) -> AgentTaskResponse:
        """
    ...
    async def get_agent_workload(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_agents = sum(
                1 for a in agents if a.status == AgentStatus.ACTIVE.value
            )
            idle_agents = sum(1 for a in agents if a.status == AgentStatus.IDLE.value)
            offline_agents = sum(
                1 for a in agents if a.status == AgentStatus.OFFLINE.value
            )
            ...
            return {
                ...
                "avg_success_rate": (
                    float(sum(a.success_rate for a in agents) / total_agents)
                    if total_agents > 0
                    else 0.0
                ),
            }
