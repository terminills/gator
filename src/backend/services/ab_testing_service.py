...
class ABTestingService:
        ...
        try:
            ...
            self.test_results[config.test_id] = {
                v["variant_id"]: VariantPerformance(
                    variant_id=v["variant_id"], name=v.get("name", v["variant_id"])
                )
                for v in variants
            }
    ...
    async def record_variant_event(
        self, test_id: UUID, variant_id: str, event_type: str, value: float = 1.0
    ) -> bool:
        """
    ...
    async def analyze_test(
        self, test_id: UUID, auto_select_winner: bool = True
    ) -> ABTestResult:
        """
        ...
        try:
                ...
                variant_analyses.append(
                    {
                        ...
                        "vs_control": (
                            f"{((variant_metric_value / best_performance - 1) * 100):.1f}%"
            ...
            if winner and winner != "control":
                ...
                insights.append(
                    "Control performs best - no variants show significant improvement"
                )
            ...
            if winner and winner != "control":
                recommendation = (
                    f"Deploy variant '{winner}' - shows significant improvement"
                )
            elif winner == "control":
                recommendation = (
                    "Maintain control - test variants did not improve performance"
                )
            else:
            ...
            winner_analysis = (
                next((v for v in variant_analyses if v["variant_id"] == winner), None)
                if winner and winner != "control"
                else None
            )
    ...
    async def get_test_status(self, test_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            required_impressions = len(results) * config.minimum_sample_size
            progress_pct = min(
                100,
                (
                    (total_impressions / required_impressions * 100)
                    if required_impressions > 0
                    else 0
                ),
            )
