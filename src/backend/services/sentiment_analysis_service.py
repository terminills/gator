...
class SentimentAnalysisService:
    ...
    async def analyze_social_comments(
        self, comments: List[Dict[str, Any]], persona_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def analyze_engagement_sentiment(
        self, post_data: Dict[str, Any], engagement_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        content_sentiment = await self.analyze_text(
            post_data.get("content", ""), context={"type": "post_content"}
        )
    ...
    async def track_sentiment_trends(
        self, persona_id: str, time_range: timedelta = timedelta(days=30)
    ) -> Dict[str, Any]:
        """
    ...
    async def compare_competitor_sentiment(
        self, persona_id: str, competitor_ids: List[str]
    ) -> Dict[str, Any]:
        """
    ...
    def _detect_emotions(self, tokens: List[str]) -> Dict[str, float]:
        ...
        if total > 0:
            emotion_scores = {
                emotion: score / total for emotion, score in emotion_scores.items()
            }
    ...
    def _extract_topics(self, tokens: List[str]) -> List[str]:
        ...
        topics = [
            ...
            for token in tokens
            if len(token) > 5
            and token not in self.positive_words
            and token not in self.negative_words
        ]
    ...
    def _classify_intent(self, text: str) -> str:
        ...
        if any(
            word in text_lower for word in ["?", "how", "what", "when", "where", "why"]
        ):
            return "question"
        elif any(word in text_lower for word in ["thank", "thanks", "appreciate"]):
            return "appreciation"
        elif any(
            word in text_lower for word in ["complain", "issue", "problem", "fix"]
        ):
            return "complaint"
        elif any(word in text_lower for word in ["recommend", "suggest", "should"]):
            return "recommendation"
        elif any(
            word in text_lower for word in ["love", "amazing", "great", "awesome"]
        ):
            return "praise"
        else:
            return "statement"
    ...
    def _aggregate_sentiment_results(
        ...
        return {
            ...
            "positive_ratio": sum(1 for r in results if r["sentiment_score"] > 0.2)
            / len(results),
            "negative_ratio": sum(1 for r in results if r["sentiment_score"] < -0.2)
            / len(results),
        }
    ...
    def _generate_recommendations(
        self, sentiment_score: float, trend: str
    ) -> List[str]:
        ...
        if trend == "declining":
            recommendations.append(
                "Review recent content changes that may have impacted sentiment"
            )
