                    ...
                    for preferred in OLLAMA_MODELS:
                        for available in available_models:
                            if preferred in available.lower():
                                ...
                                logger.info(f"Ollama available with model: {self._available_model}")
    ...
    async def generate_persona(
        ...
        try:
            ...
            if use_ai and await self._check_ollama_available():
                logger.info(f"Generating AI-powered persona with Ollama (type: {persona_type or 'general'})")
                return await self._generate_with_ai(
                    name=name,
                    persona_type=persona_type
                )
            else:
                logger.info("Using template-based persona generation (Ollama unavailable)")
