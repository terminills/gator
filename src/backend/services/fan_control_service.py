        ...
        async with database_manager.get_session() as session:
            ...
            if any([ipmi_host, ipmi_username, ipmi_password, ipmi_interface]):
                return {
                    ...
                    "ipmi_interface": (
                        ipmi_interface.value if ipmi_interface else "lanplus"
                    ),
                }
...
class FanControlService:
    ...
    def __init__(
        ...
        auth_status = (
            "with authentication"
            if self._ipmi_host and self._ipmi_username
            else "without authentication"
        )
        logger.info(
            f"Fan control service initialized for manufacturer: {manufacturer.value} {auth_status}"
        )
    ...
    def _build_ipmi_command(self, raw_command: List[str]) -> List[str]:
        ...
        if self._ipmi_host and self._ipmi_username and self._ipmi_password:
            cmd.extend(
                [
                    "-I",
                    self._ipmi_interface,  # Interface (lanplus for remote)
                    "-H",
                    self._ipmi_host,  # BMC/XCC hostname or IP
                    "-U",
    ...
    async def _set_ipmi_fan_mode_auto(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"Setting auto mode for {self._manufacturer.value}: {' '.join(auto_cmd)}"
            )
            ...
            if proc.returncode == 0:
                ...
                if "Invalid command" in error_msg or "0xc1" in error_msg.lower():
                    ...
                    logger.warning(
                        f"IPMI auto mode command returned error: {error_msg}"
                    )
    ...
    async def set_fan_speed(
        self, speed_percent: int, zone: Optional[FanZone] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self._control_mode != FanControlMode.MANUAL:
                ...
                if not mode_result.get("success"):
                    logger.warning(
                        "Could not set manual mode, continuing with speed setting attempt"
                    )
            ...
            self._get_zone_id(zone)

            # Build the full command with speed value
            speed_cmd = list(speed_cmd_prefix) + [hex(raw_speed)]

            logger.debug(
                f"Setting fan speed for {self._manufacturer.value}: {' '.join(speed_cmd)}"
            )
            ...
            if proc.returncode == 0:
                ...
                if self._is_authentication_error(error_msg):
                    ...
                    auth_configured = bool(
                        self._ipmi_host and self._ipmi_username and self._ipmi_password
                    )
                    return {
                        "success": False,
                        "error": "IPMI authentication failed",
                        "details": (
                            "Unable to authenticate with BMC/XCC. Check credentials and ensure IPMI over LAN is enabled."
                            if auth_configured
                            else "IPMI credentials not configured. Remote IPMI access requires authentication."
                        ),
                        "ipmi_error": error_msg,
                        "recommendation": (
                            "Configure IPMI credentials (host, username, password) and ensure IPMI over LAN is enabled in XCC/BMC settings"
                            if not auth_configured
                            else "Verify IPMI credentials are correct and IPMI over LAN is enabled in XCC/BMC settings"
                        ),
    ...
    async def _set_ipmi_fan_mode_manual(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"Setting manual mode for {self._manufacturer.value}: {' '.join(manual_cmd)}"
            )
            ...
            if proc.returncode == 0:
                ...
                if "Invalid command" in error_msg or "0xc1" in error_msg.lower():
                    logger.info(
                        "IPMI raw fan control commands not supported on this hardware"
                    )
                    return {
                        "success": False,
                    ...
                    logger.warning(
                        f"IPMI manual mode command returned error: {error_msg}"
                    )
    ...
    async def adjust_fans_for_temperature(self, max_gpu_temp: float) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                self._control_mode == FanControlMode.AUTO
                or max_gpu_temp >= self._temperature_thresholds["critical"]
            ):
                ...
                if (
                    ...
                    and self._manual_control_supported is False
                ):
                    logger.info(
    ...
    def _calculate_fan_speed(self, temperature: float) -> int:
        ...
        if temperature < thresholds["low"]:
            ...
            ratio = (temperature - thresholds["low"]) / (
                thresholds["normal"] - thresholds["low"]
            )
            ...
            ratio = (temperature - thresholds["normal"]) / (
                thresholds["high"] - thresholds["normal"]
            )
            ...
            ratio = (temperature - thresholds["high"]) / (
    ...
    def set_manufacturer(self, manufacturer: ServerManufacturer) -> Dict[str, Any]:
        ...
        logger.info(
            f"Manufacturer changed from {old_manufacturer.value} to {manufacturer.value}"
        )
    ...
    def set_ipmi_credentials(
        self, host: str, username: str, password: str, interface: str = "lanplus"
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"IPMI credentials updated for host: {host} (interface: {interface})"
        )
    ...
    def get_control_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "authentication_mode": (
                "remote (out-of-band)" if credentials_configured else "local (in-band)"
            ),
...
def get_fan_control_service(
    manufacturer: Optional[ServerManufacturer] = None,
) -> FanControlService:
    """
