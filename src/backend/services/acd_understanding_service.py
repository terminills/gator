...
class ACDUnderstandingService:
    ...
    async def _get_error_analysis(self, cutoff_time: datetime) -> Dict[str, Any]:
        ...
        try:
            stmt = select(ACDTraceArtifactModel).where(
                ACDTraceArtifactModel.timestamp >= cutoff_time
    ...
    async def _get_engagement_metrics(self, cutoff_time: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            performance_scores = [p.compared_to_average for p in posts if p.compared_to_average]
            ...
            top_posts = sorted(posts, key=lambda p: (p.engagement_rate or 0), reverse=True)[:5]
            top_performing = [
                {
                    "id": str(p.id),
                    "platform": p.platform,
            ...
            return {
                ...
                "interaction_quality": {
                    ...
                    "genuine_ratio": round(genuine_users / max(genuine_users + bot_interactions, 1) * 100, 1),
                },
    ...
    async def _get_ppv_performance(self, cutoff_time: datetime) -> Dict[str, Any]:
        ...
        """
        try:
            stmt = select(PPVOfferModel).where(
            ...
            avg_price = sum(float(o.price or 0) for o in offers) / total if total > 0 else 0
            avg_accepted_price = total_revenue / len(accepted_offers) if accepted_offers else 0
    ...
    async def _get_user_activity(self, cutoff_time: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "engagement_depth": round(len(user_messages) / max(len(active_user_ids), 1), 1),
            }
        ...
        try:
            ...
            return {
                ...
                "reach_metrics": {
                    ...
                    "avg_impressions_per_post": round(total_impressions / max(posts_published, 1), 1),
                },
                "content_velocity": round(content_created / max(24, 1), 2),  # per hour average
            }
    ...
    async def _get_churn_indicators(self, cutoff_time: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            if ppv_offers:
                accepted = sum(1 for o in ppv_offers if o.status == PPVOfferStatus.ACCEPTED)
                total_responded = sum(1 for o in ppv_offers if o.status in [PPVOfferStatus.ACCEPTED, PPVOfferStatus.DECLINED])
            ...
            if churn_rate > 30:
                risk_level = "HIGH"
                risk_factors.append(f"High churn rate: {round(churn_rate, 1)}% inactive")
            elif churn_rate > 15:
                risk_level = "MEDIUM"
            ...
            if ppv_acceptance_rate < 10 and ppv_offers:
                risk_factors.append(f"Low PPV conversion: {round(ppv_acceptance_rate, 1)}%")
    ...
    async def recall_context(self, context_id: UUID) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            artifact_stmt = select(ACDTraceArtifactModel).where(
                ACDTraceArtifactModel.acd_context_id == context_id
            ).order_by(desc(ACDTraceArtifactModel.timestamp))
...
and error patterns. This enables intelligent recommendations.""",
    ...
    async def _analyze_timing_patterns(
        ...
        for hour in completions_by_hour:
            ...
            success_rate_by_hour[hour] = round(successes / total * 100, 1) if total > 0 else 0
        
        # Find optimal hours (highest success rate with sufficient volume)
        optimal_hours = []
        for hour, rate in sorted(success_rate_by_hour.items(), key=lambda x: x[1], reverse=True):
                ...
                "next_scheduled": [
                    {
                        "id": str(ctx.id),
                        "phase": ctx.ai_phase,
                        "scheduled_for": ctx.scheduled_for.isoformat() if ctx.scheduled_for else None,
        ...
        for ctx in contexts:
            if ctx.orchestration_group:
                ...
                if group not in groups:
                    groups[group] = {"total": 0, "completed": 0, "in_progress": 0, "queued": 0}
        ...
        try:
            ...
            for ctx in contexts:
                learning_data.append({
                    ...
                    "optimization_goal": ctx.schedule_optimization_goal,
                    "scheduled_for": ctx.scheduled_for.isoformat() if ctx.scheduled_for else None,
                    "actual_start": ctx.ai_started.isoformat() if ctx.ai_started else None,
            ...
            scores = [d["effectiveness_score"] for d in learning_data if d["effectiveness_score"]]
