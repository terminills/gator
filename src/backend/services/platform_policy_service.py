...
class PlatformPolicyService:
    ...
    async def _load_cache(self) -> None:
        ...
        stmt = select(PlatformPolicyModel).where(
            PlatformPolicyModel.is_active.is_(True)
        )
        result = await self.db.execute(stmt)
        policies = result.scalars().all()

        self._policy_cache = {
            policy.platform_name.lower(): policy for policy in policies
        }
    ...
    async def create_platform_policy(
        ...
        if existing:
            raise ValueError(
                f"Platform policy for '{policy_data.platform_name}' already exists"
            )
