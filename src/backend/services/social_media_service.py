...
class SocialMediaService:
    ...
    async def add_account(self, account: SocialAccount) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Social media account added platform={account.platform} account_id={account.account_id}"
            )
            return True
    ...
    async def publish_content(self, request: PostRequest) -> List[PostResponse]:
        ...
        try:
            ...
            for platform in request.platforms:
                ...
                logger.info(
                    f"Content published to platform content_id={request.content_id} platform={platform} status={response.status}"
                )
            ...
            logger.error(
                f"Failed to publish content: {str(e)} content_id={request.content_id}"
            )
        ...
        try:
            ...
            for platform in request.platforms:
                ...
                task = publish_scheduled_post.apply_async(
                    args=[schedule_id, post_data], eta=request.schedule_time
                )

                logger.info(
                    f"Post scheduled with Celery: schedule_id={schedule_id} platform={platform} schedule_time={request.schedule_time} task_id={task.id}"
                )
    ...
    async def get_engagement_metrics(
        self, post_id: str, platform: PlatformType
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error(
                f"Failed to get engagement metrics: {str(e)} post_id={post_id} platform={platform}"
            )
            return {"error": str(e)}
    ...
    async def _publish_to_platform(
        ...
        try:
            ...
            post_data = await self._prepare_post_data(
                content, persona, request, platform
            )
            ...
            return PostResponse(
                ...
                status=(
                    PostStatus.PUBLISHED if result.get("success") else PostStatus.FAILED
                ),
    ...
    def _generate_hashtags(
        self, content: ContentResponse, persona: PersonaModel
    ) -> List[str]:
