...
class AgentMarketplaceService:
        ...
        try:
            ...
            if query:
                ...
                results = [
                    ...
                    if (
                        query_lower in agent.agent_name.lower()
            ...
            if agent_type:
                results = [
                    agent for agent in results if agent.agent_type == agent_type.value
                ]
            ...
            if capabilities:
                ...
                results = [
                    agent
                    for agent in results
                    if capability_set.issubset(
                        set(c.lower() for c in agent.capabilities)
                    )
                ]
            ...
            if free_only:
                results = [agent for agent in results if agent.price == 0.0]
    ...
    async def install_agent(
        self, agent_id: UUID, custom_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not marketplace_entry:
                return {"success": False, "error": "Agent not found in marketplace"}
    ...
    async def check_updates(self, agent_id: UUID) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for entry in self.marketplace_catalog.values():
                if (
                    entry.agent_name == agent.agent_name
                    and entry.agent_type == agent.agent_type
                ):
                ...
                installed.append(
                    {
                        ...
                        "author": (
                            agent.plugin_author if agent.is_plugin else "Built-in"
                        ),
                        "status": agent.status,
                        "updates_available": update_info.get(
                            "updates_available", False
                        ),
    ...
    async def publish_agent(self, agent_entry: AgentMarketplaceEntry) -> Dict[str, Any]:
        """
    ...
    async def get_marketplace_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get top agents
            top_rated = sorted(
                self.marketplace_catalog.values(), key=lambda x: x.rating, reverse=True
            )[:5]
