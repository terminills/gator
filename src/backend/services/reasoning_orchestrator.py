...
class ReasoningOrchestrator:
    ...
    async def orchestrate_decision(
        ...
        try:
            ...
            logger.info(
                "🧠 Calling reasoning engine for intelligent decision making..."
            )
            ...
            if decision.decision_type in [
                ...
                logger.info(
                    f"🔧 Decomposing task for {decision.decision_type.value}..."
                )
    ...
    async def _assess_situation(
        ...
        if context.ai_started:
            time_spent = (
                datetime.now(timezone.utc) - context.ai_started
            ).total_seconds()
    ...
    async def _query_relevant_patterns(
        ...
        try:
            ...
            if (
                ...
                and (datetime.now(timezone.utc) - self._last_cache_update)
                < self._cache_timeout
            ):
            ...
            if context.ai_domain:
                try:
                    from backend.models.acd import DOMAIN_COMPATIBILITY, AIDomain
                    ...
                    domain_filter = [context.ai_domain] + [
                        d.value for d in compatible_domains
                    ]
                    stmt = stmt.where(ACDContextModel.ai_domain.in_(domain_filter))
                    logger.info(
                        f"Filtering patterns by domain: {context.ai_domain} (+ {len(compatible_domains)} compatible)"
                    )
                except Exception as e:
    ...
    async def _select_action(
        ...
        if (
            situation["is_blocked"]
            and situation["time_spent_seconds"]
            and situation["time_spent_seconds"] > 300
        ):
            return OrchestrationDecision(
        ...
        if (
            context.ai_confidence == AIConfidence.UNCERTAIN.value
        ...
        if (
            situation["has_errors"]
            and patterns
            and situation["retry_count"] < self.retry_limit
        ):
            return OrchestrationDecision(
                ...
                action_plan={
                    ...
                    "apply_patterns": True,
                    "strategies": [
                        p.get("strategy") for p in patterns[:3] if p.get("strategy")
                    ],
                },
            )
    ...
    async def _execute_collaboration_request(
        ...
        try:
            update = ACDContextUpdate(
                ...
                ai_metadata={
                    "collaboration_type": decision.action_plan.get(
                        "collaboration_type"
                    ),
    ...
    async def _execute_retry(
        ...
        try:
            ...
            current_metadata["learned_strategies"] = decision.action_plan.get(
                "strategies", []
            )
    ...
    async def learn_from_outcome(
        ...
        try:
            ...
            if (
                not context.ai_metadata
                or "orchestration_decision" not in context.ai_metadata
            ):
                return
            ...
            if success:
                ...
                current_metadata["failed_decisions"].append(
                    {
                        ...
                        "reason": (
                            outcome_metadata.get("failure_reason")
                            if outcome_metadata
                            else None
                        ),
                    }
                ...
                await self.acd_service.update_context(
                    context_id, ACDContextUpdate(ai_metadata=current_metadata)
                )
