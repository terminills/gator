    ...
    async def aggregate_engagement_patterns(
        ...
        try:
            ...
            for context in contexts:
                ...
                persona_key = str(context.content_id or context.benchmark_id or "unknown")
                ...
                if engagement_rate > 0:
                    persona_patterns[persona_key]["engagement_rates"].append(engagement_rate)
            ...
            for hour, count in hour_counts.items():
                noisy_count = max(0, int(self._add_laplace_noise(count, sensitivity=1.0)))
                if noisy_count > 0:
                    noisy_hour_dist[hour] = noisy_count
            
            # Calculate top hours from noisy distribution
            sorted_hours = sorted(
                noisy_hour_dist.items(),
                key=lambda x: x[1],
                reverse=True
            )
            ...
            return {
                ...
                "aggregate_patterns": {
                    ...
                    "hour_distribution": {
                        str(h): int(c) for h, c in sorted_hours
                    },
    ...
    async def get_benchmarked_performance(
        ...
        try:
            ...
            if not agg_patterns.get("success"):
                return {
                    "success": False,
                    "reason": "insufficient_aggregate_data"
                }
            ...
            return {
                ...
                "comparison": {
                    ...
                    "tier": tier,
                    "vs_average": f"{((persona_mean / aggregate_mean - 1) * 100):.1f}%"
                        if aggregate_mean > 0 else "N/A",
