...
async def check_column_exists(conn, table_name: str, column_name: str, is_sqlite: bool) -> bool:
    """
...
async def add_personas_appearance_columns(conn, is_sqlite: bool) -> List[str]:
    ...
    if not await check_column_exists(conn, "personas", "base_appearance_description", is_sqlite):
    ...
    if not await check_column_exists(conn, "personas", "appearance_locked", is_sqlite):
        ...
        try:
            ...
            await conn.execute(
                text("CREATE INDEX IF NOT EXISTS ix_personas_appearance_locked ON personas (appearance_locked)")
            )
    ...
    if not await check_column_exists(conn, "personas", "base_image_status", is_sqlite):
        ...
        if is_sqlite:
            await conn.execute(
                text("ALTER TABLE personas ADD COLUMN base_image_status VARCHAR(20) DEFAULT 'pending_upload'")
            )
            # Update existing rows to have the default value
            await conn.execute(
                text("UPDATE personas SET base_image_status = 'pending_upload' WHERE base_image_status IS NULL")
            )
        else:
            await conn.execute(
                text("ALTER TABLE personas ADD COLUMN base_image_status VARCHAR(20) NOT NULL DEFAULT 'pending_upload'")
            )
        ...
        try:
            ...
            await conn.execute(
                text("CREATE INDEX IF NOT EXISTS ix_personas_base_image_status ON personas (base_image_status)")
            )
        except Exception as e:
    ...
    try:
        async with engine.begin() as conn:
            ...
            if columns_added:
                ...
                logger.info(f"Added {len(columns_added)} column(s) to personas table: {', '.join(columns_added)}")
            else:
