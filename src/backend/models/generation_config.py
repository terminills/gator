...
class ImageGenerationConfig(BaseModel):
    ...
    resolution: ImageResolution = Field(
        default=ImageResolution.HD_1024, description="Output image resolution"
    )
    ...
    quality: QualityPreset = Field(
        default=QualityPreset.HIGH, description="Generation quality preset"
    )
    ...
    guidance_scale: Optional[float] = Field(
        default=None, ge=1.0, le=20.0, description="Guidance scale for generation"
    )
...
class VideoGenerationConfig(BaseModel):
    ...
    resolution: VideoResolution = Field(
        default=VideoResolution.FHD_1080, description="Output video resolution"
    )
    custom_width: Optional[int] = Field(
        default=None,
        ge=256,
    ...
    duration: float = Field(
        default=3.0, ge=0.5, le=30.0, description="Video duration in seconds"
    )
    quality: QualityPreset = Field(
        default=QualityPreset.HIGH, description="Generation quality preset"
    )
...
def get_resolution_from_dimensions(
...
def get_recommended_resolutions_for_platform(platform: str) -> list[ImageResolution]:
    ...
    return platform_resolutions.get(
        platform.lower(), [ImageResolution.LANDSCAPE_FHD, ImageResolution.HD_1024]
    )
