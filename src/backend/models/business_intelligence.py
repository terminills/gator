...
class TrafficMetricsModel(Base):
    ...
    # Traffic sources
    source_platform = Column(String(50), nullable=False, index=True)  # TrafficSource enum
...
class UserRetentionModel(Base):
    ...
    # Persona preferences
    favorite_personas = Column(JSON, nullable=True)  # List of persona IDs with interaction counts
...
class ContentScheduleModel(Base):
    ...
    platform = Column(String(50), nullable=False, index=True)  # instagram, twitter, etc.
    ...
    # Status
    status = Column(String(20), default="scheduled", index=True)  # scheduled, processing, posted, failed, cancelled
...
class SchedulingFeedbackModel(Base):
    ...
    audience_state_snapshot = Column(JSON, nullable=True)  # Active users, recent engagement
    ...
    decision_source = Column(String(50), nullable=True)  # Was it LLM, rule-based, manual?
    decision_source_score = Column(Float, nullable=True)  # How well did the source perform?
