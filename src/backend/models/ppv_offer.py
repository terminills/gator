...
class PPVOfferModel(Base):
    ...
    offer_type = Column(SQLEnum(PPVOfferType), nullable=False, index=True)
    status = Column(
    ...
    estimated_delivery_hours = Column(
        Integer, default=24
    )  # How long to deliver content
...
class PPVOfferCreate(BaseModel):
    ...
    description: str = Field(
        min_length=10, max_length=2000, description="Offer description"
    )
    ...
    preview_url: Optional[str] = Field(
        None, max_length=500, description="Preview content URL"
    )
    content_metadata: Optional[Dict[str, Any]] = Field(
        default={}, description="Additional content metadata"
    )
    estimated_delivery_hours: int = Field(
        default=24, ge=1, le=168, description="Delivery time in hours"
    )
    delivery_instructions: Optional[str] = Field(
        None, max_length=1000, description="Delivery instructions"
    )
