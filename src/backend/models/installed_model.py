...
class InstalledModelModel(Base):
    ...
    huggingface_filename = Column(
        String(255), nullable=True
    )  # Specific file in the repo
    ...
    trigger_words = Column(
        JSON, nullable=False, default=list
    )  # ["keyword1", "keyword2"]
    trained_words = Column(
        JSON, nullable=True, default=list
    )  # Original trained words from CivitAI
    ...
    )  # For any additional CivitAI data
...
class InstalledModelCreate(BaseModel):
    ...
    source: str = Field(
        default="local", description="Source (civitai, huggingface, local)"
    )

    file_path: str = Field(
        min_length=1, max_length=1000, description="Full path to model file"
    )
