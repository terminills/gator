...
class PersonalizedContentModel(Base):
    ...
    segment_id = Column(UUID(as_uuid=True), ForeignKey("audience_segments.id"), nullable=False, index=True)
...
class SegmentMemberModel(Base):
    ...
    # Membership details
    confidence_score = Column(Float, default=1.0)  # How confident we are in this assignment
    assignment_reason = Column(JSON, nullable=True)  # Why user was assigned to this segment
...
class AudienceSegmentCreate(BaseModel):
    ...
    strategy: PersonalizationStrategy = Field(PersonalizationStrategy.HYBRID, description="Personalization strategy")
...
class PersonalizedContentCreate(BaseModel):
    ...
    variant_id: Optional[str] = Field(None, max_length=50, description="A/B test variant identifier")
