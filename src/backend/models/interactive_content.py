...
class InteractiveContentModel(Base):
    ...
    options = Column(JSON, nullable=True)  # For polls: [{"id": 1, "text": "Option 1", "votes": 0}]
...
class InteractiveContentResponse(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    content_id = Column(UUID(as_uuid=True), ForeignKey("interactive_content.id"), nullable=False, index=True)
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True, index=True)  # Nullable for anonymous
    response_data = Column(JSON, nullable=False)  # Flexible structure for different content types
    user_identifier = Column(String(100), nullable=True)  # For anonymous tracking (hashed IP, session ID, etc.)
    created_at = Column(DateTime, default=func.now(), nullable=False)
    ...
    def __repr__(self):
        return f"<InteractiveContentResponse(id={self.id}, content_id={self.content_id})>"
...
class InteractiveContentCreate(BaseModel):
    ...
    content_type: InteractiveContentType = Field(..., description="Type of interactive content")
    ...
    options: Optional[List[Dict[str, Any]]] = Field(None, description="Options for polls")
    media_url: Optional[str] = Field(None, max_length=500, description="Media URL for stories")
...
class InteractiveContentResponseCreate(BaseModel):
    ...
    response_data: Dict[str, Any] = Field(..., description="Response data (structure varies by content type)")
    user_id: Optional[str] = Field(None, description="User ID (for authenticated responses)")
